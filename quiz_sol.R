...
data.table("population_country.csv", package = "data.table")
...
df_sub = df_sub[df$`Country Name` != "World", ]
...
as.data.table(df_sub)

# 9. df_sub 객체를 data.table 패키지의 melt() 함수를 사용하여 국가명[Country_Name], 
#연도[year], 인구수[pop] 정보를 담고있는 df_melt 객체를 생성하시오.\
df_melt = melt(df_sub, id.vars = c("Country_Name", 
                                   "year",
                                   "pop"))
...
# ※ clist와 df_slope 객체 활용 권장 ※ 인구 증가율은 데이터가 있는 연도를 기준으로(1980부터 기록된 경우 1980년 부터 2016년 까지) 계산하시오. ※ 국가별 데이터가 기록된 최초 연도를 기준으로 그 증가율을 계산하시오.